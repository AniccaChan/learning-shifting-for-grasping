...
class Pushing : public Agent {
  ...
  double getMaxGraspProbAroundPose(const std::vector<DepthImage>& images,
                                   const RobotPose& pose) {
    return grasp_inference
        ->inferAroundPose(images[0], pose, SelectionMethod::Max)
        .prob;
  }

 public:
  explicit Pushing(const std::string& config_filename)
    ...
    if (config.use_cpp_inference) {
      grasp_inference = std::make_shared<Inference>(
          config.secondary_model + "-sm", config.bin_data);
    }
  }
  ...
  void reset() {
    ...
    robot->moveRelativeCartesian(
        config.gripper_frame,
        relativeSafePose(robot->currentPose(config.gripper_frame)),
        MotionData().withDynamics(0.3));
    robot->moveJoints(config.bin_joint_values.at(current_bin),
                      MotionData().withDynamics(0.7));
  ...
  void runEpisode(Epoch epoch, int current_episode_in_epoch) {
    ...
    if (config.mode != Mode::Perform) {
      robot->moveJoints(config.bin_joint_values.at(current_bin));
      robot->moveCartesian(config.camera_frame,
                           getBinCameraAffine(current_bin));
    }
    ...
    if (config.mode == Mode::Measure) {
      if (epoch.selection_method_primary == SelectionMethod::Top5 ||
          epoch.selection_method_primary == SelectionMethod::Bottom5) {
        epoch.selection_method_primary = (max_grasp_prob < 0.7)
                                             ? SelectionMethod::Top5
                                             : SelectionMethod::Bottom5;
      }
    ...
    if (config.use_cpp_inference) {
      InferenceResult inference_result =
          push_inference->infer(images[0], current_method);
    ...
    std::chrono::steady_clock::time_point end =
        std::chrono::steady_clock::now();
    std::cout << "Image + inference detection time [ms]: "
    ...
    for (const DepthImage image : images) {
      auto image_future =
          push_io->uploadImage(push_result.id, image, image.suffix + "-v");
      image_future.wait();
    }
    ...
    if (new_push.found == 0) {
      ...
      Eigen::Affine3d new_grasp_approach_frame =
          Affine(0, 0, config.approach_distance_from_pose) * new_grasp_frame;

      robot->moveCartesian(config.gripper_frame, new_grasp_approach_frame);

      auto approach_motion_data =
          MotionData()
              .withDynamics(config.approach_dynamics_rel)
              .withZForceCondition(6.0);  // [N]
      robot->moveRelativeCartesian(
          config.gripper_frame,
          Affine(0.0, 0.0, -config.approach_distance_from_pose),
          approach_motion_data);
      ...
      push_result.start = RobotPose(image_frame.inverse() *
                                    robot->currentPose(config.gripper_frame));

      // Actual push movement
      Eigen::Affine3d push_transformation =
          Affine(config.push_distance, 0.0, 0.0);
      ...
      auto push_motion_data = MotionData()
                                  .withDynamics(config.approach_dynamics_rel)
                                  .withZForceCondition(10.0);  // [N]
      robot->moveRelativeCartesian(config.gripper_frame, push_transformation,
                                   push_motion_data);
      robot->recoverFromErrors();

      push_result.end = RobotPose(image_frame.inverse() *
                                  robot->currentPose(config.gripper_frame));

      auto up_motion_data =
          MotionData().withDynamics(0.7).withZForceCondition(15.0);  // [N]
      bool move_up_execution_successful = robot->moveRelativeCartesian(
          config.gripper_frame,
          Affine(0.0, 0.0, config.approach_distance_from_pose), up_motion_data);
      ...
      for (const DepthImage image : images_after) {
        auto image_future = push_io->uploadImage(push_result.id, image,
                                                 image.suffix + "-after");
      }
      push_result.probend =
          getMaxGraspProbAroundPose(images_after, new_push.pose);
    ...
    // Change bin
    if (count_episodes_in_current_bin >=
