...
struct GraspConverter {
  ...
  bool checkSafety(Grasp& grasp) {
    bool inside_bin = bin.isPoseInside(grasp.pose) ||
                      !pose_should_be_inside_bin;  // so insideBin only if pose
                                                   // should be inside bin
  ...
  double calculatePoseZ(const DepthImage& image, const RobotPose& pose) {
    DepthImage area_image =
        dimg::getAreaOfInterest(image, pose, cv::Size{752, 480},
                                bin.background_color, cv::INTER_NEAREST);

    // Get distance at gripper for possible collisions
    const double gripper_one_side_size =
        0.5 * image.pixel_size * (pose.d + 0.002);  // [px]
    cv::Size side_gripper_image_size(image.pixel_size * 0.024,
                                     image.pixel_size * 0.024);

    DepthImage area_center = dimg::crop(
        area_image, cv::Size(image.pixel_size * 0.01, image.pixel_size * 0.03));
    DepthImage area_center_sm = dimg::crop(
        area_image,
        cv::Size(image.pixel_size * 0.003, image.pixel_size * 0.003));
    DepthImage area_left = dimg::crop(area_image, side_gripper_image_size,
                                      cv::Point2d(-gripper_one_side_size, 0));
    DepthImage area_right = dimg::crop(area_image, side_gripper_image_size,
                                       cv::Point2d(gripper_one_side_size, 0));
    ...
    cv::minMaxLoc(area_center_sm.image, &min_area_center_sm,
                  &max_area_center_sm);
    ...
    cv::minMaxLoc(area_left.image, &min_area_left, &max_area_left, 0, 0,
                  area_left_mask);
    double z_raw_left = area_left.depthFromValue(min_area_left);

    double min_area_right, max_area_right;
    cv::minMaxLoc(area_right.image, &min_area_right, &max_area_right, 0, 0,
                  area_right_mask);
  ...
  InferenceResult convertBack(const Grasp& grasp) {
    ...
    auto const it = std::min_element(
        gripper_classes.begin(), gripper_classes.end(),
        [=](double x, double y) {
